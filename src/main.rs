...
fn main() {
	...
	// img.apply(RandomPointsNoise::new_random_default(image_wh_f));
	// img.apply_with_force((
	// 	RandomPointsNoise::new_random_default(image_wh_f),
	// 	RandomPointsNoise::new_random_default(image_wh_f),
	// 	RandomPointsNoise::new_random_default(image_wh_f),
	// ), 0.5);
	...
	img.apply_with_force(PullInWithAngleAndForce {
		pull_from: PullIn::AllArea,
		angle: cli_args.angle_of_rays.unwrap_or_else(|| AngleDeg::new_random()),
		pull_force: rng.gen_range(0.95 ..= 0.999),
		pixels_iter_order: PixelsIterOrder::new_random(),
		// pixels_iter_order: PixelsIterOrder::Default,
	}, 1.0);
...
impl Apply<ShapeWithColor> for ImageBuf {
	fn apply_with_force(&mut self, ShapeWithColor { shape, color }: ShapeWithColor, force: float) {
		let color = color.to_image_rgb();
		let w_half = self.width()  as float / 2.;
		let h_half = self.height() as float / 2.;
...
impl PixelsIterOrder {
	pub fn new_random() -> Self {
		...
		match thread_rng().gen_typesafe_with_weights([1, 10,5, 1,1,1,1, 1,1,1,1]) {
			...
			_1 => Default2,
			_2 => Random,
			_3 => X_Y, _4 => Xinv_Y, _5 => X_Yinv, _6  => Xinv_Yinv,
			_7 => Y_X, _8 => Yinv_X, _9 => Y_Xinv, _10 => Yinv_Xinv,
		}
	}
